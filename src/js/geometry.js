...
(function (A) {
  ...
  geom.openPath = function (strokes) {
    ...
    if (keep.length === 1) return keep[0].slice();
    // several real strokes (curve drawn in pieces) → greedy-chain by nearest end
    const rem = keep.slice().sort((a, b) => (a[0][0] + a[0][1]) - (b[0][0] + b[0][1]));
    let path = rem.shift().slice();
    while (rem.length) {
      const tail = path[path.length - 1];
      let bi = 0, flip = false, bd = Infinity;
      for (let i = 0; i < rem.length; i++) {
        const s = rem[i];
        const dS = geom.dist(tail, s[0]), dE = geom.dist(tail, s[s.length - 1]);
        if (dS < bd) { bd = dS; bi = i; flip = false; }
        if (dE < bd) { bd = dE; bi = i; flip = true; }
      }
      let s = rem.splice(bi, 1)[0];
      path = path.concat(flip ? s.slice().reverse() : s);
    }
    return path;
  };
